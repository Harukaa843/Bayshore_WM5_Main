			...
			// Declare some variable
				...
				switch (body.gameMode) 
				{
					...
					case wm.wm5.protobuf.GameMode.MODE_GHOST_BATTLE:
					{
						...
						// OCM Mode Play
						if((body.rgResult as any)?.ocmMode)
						{
							competitionModePlay = true;

							// Calling save OCM ghost battle result function (BASE_PATH/src/modules/ghost/ghost_history.ts)
							let ghostResult = await ghost_history.saveOCMGhostHistory(body);
							updateNewTrail = ghostResult.updateNewTrail;
						}
						// Crown Mode Play
						else if((body.rgResult as any)?.crownBattle)
						{
							crownModePlay = true;
							updateNewTrail = true;

							// Calling save ghost battle result function (BASE_PATH/src/modules/ghost/ghost_history.ts)
							await ghost_history.saveGhostHistory(body);
						}
						// Normal Ghost Mode Play
						else
						{
							updateNewTrail = true;
